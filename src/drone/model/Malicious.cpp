...
{
	static TypeId tid =
		TypeId("ns3::Malicious")
			.SetParent<Application>()
			.SetGroupName("Applications")
			.AddConstructor<Malicious>()
			.AddAttribute("PacketSize",
						  "The size of the malicious packet sent to neighbors.",
						  UintegerValue(4),
			...
			.AddAttribute("MyPRN",
						  "PRN used by malicious UAV",
						  UintegerValue(4294967295),
						  MakeUintegerAccessor(&Malicious::m_myPRN),
						  MakeUintegerChecker<uint32_t>(1))

			.AddAttribute("number_uav",
						  "number of uavs",
						  UintegerValue(0),
						  MakeUintegerAccessor(&Malicious::number_uav),
						  MakeUintegerChecker<uint32_t>(1))

			.AddAttribute("number_victim",
						  "number of victims",
						  UintegerValue(0),
						  MakeUintegerAccessor(&Malicious::number_victim),
						  MakeUintegerChecker<uint32_t>(1))
			
			.AddAttribute("GetDevice",
						  "Callback function",
						  CallbackValue(),
						  MakeCallbackAccessor(&Malicious::GetDevice),
						  MakeCallbackChecker()
						)

			.AddTraceSource("Rx",
							"Received malicious avoidance packet.",
							MakeTraceSourceAccessor(&Malicious::m_rxPacketTrace),
							"ns3::Malicious::maliciousPacketRxTracedCallback")

			.AddTraceSource("Tx",
							"Sent malicious avoidance packet.",
			...
			.AddTraceSource("DronePacketNumber",
							...
							MakeTraceSourceAccessor(&Malicious::DronePacketNumber),
							"ns3::NormalDroneApp::NormalDroneAppPacketTxTracedCallback")

			.AddTraceSource("SatelliteNumber",
							"Sent NormalDroneApp avoidance packet.",
							MakeTraceSourceAccessor(&Malicious::SatelliteNumber),
							"ns3::NormalDroneApp::NormalDroneAppPacketTxTracedCallback")

			.AddTraceSource("DroneReceivedNumber",
							"Sent NormalDroneApp avoidance packet.",
							MakeTraceSourceAccessor(&Malicious::DroneReceivedNumber),
							"ns3::NormalDroneApp::NormalDroneAppPacketTxTracedCallback")

			.AddTraceSource("SatelliteReceivedNumber",
							"Sent NormalDroneApp avoidance packet.",
							MakeTraceSourceAccessor(&Malicious::SatelliteReceivedNumber),
							"ns3::NormalDroneApp::NormalDroneAppPacketTxTracedCallback");
...
void
PlmeEdConfirm(Malicious* App, Ptr<Node> drone, LrWpanPhyEnumeration status, uint8_t level)
{
	std::cout << "Start : PlmeEdConfirm (Malicious) " << std::endl;
	std::cout << "Finish : PlmeEdConfirm (Malicious) " << std::endl;

}

void
PlmeEdConfirm_V2(Malicious* App,
				 Ptr<Node> drone,
				 ...
				 double MyPRN)
{
	std::cout << "Start : PlmeEdConfirm_V2 (Malicious) " << std::endl;
	// mousaab std::cout<<"\n_____________________PlmeEdConfirm_V2
	// (Malicious)___________________________________\n"; mousaab std::cout<<"time :" <<
	...
	std::cout << "Finish : PlmeEdConfirm_V2 (Malicious) " << std::endl;
	...
	// static_cast<uint32_t> (level)<<" and ratio ns3= "<<static_cast<double> (ratio)<<"\n"; mousaab
...
{
	std::cout << "Start : DataConfirm (Malicious) " << std::endl;
	std::cout << "Finish : DataConfirm (Malicious) " << std::endl;
...
void
DataIndication(Malicious* App,
			   Ptr<Node> drone,
			   Ptr<RandomWalk2dMobilityModel> RandomMob,
			   McpsDataIndicationParams params,
			   Ptr<Packet> p)
{
	std::cout << "Start : DataIndication (Malicious) " << std::endl;
	...
	Mac16Address dress = params.m_dstAddr;

	std::cout << "Source Address MAC 16 " << params.m_srcAddr << std::endl;
	std::cout << "Destination Address MAC 16 " << dress << std::endl;

	std::cout << "number_uav " << App->number_uav << std::endl;
	std::cout << "Address + 100 " << params.m_srcAddr.ConvertToInt()+App->number_uav << std::endl;
	std::cout << App->GetDevice(params.m_srcAddr.ConvertToInt()+App->number_uav) << std::endl;
	std::cout << App->GetDevice(params.m_srcAddr.ConvertToInt()+App->number_uav)->GetAddress() << std::endl;
	std::cout << App->GetDevice(params.m_srcAddr.ConvertToInt()+App->number_uav)->GetPhy()->GetMobility()->GetPosition() << std::endl;
	std::cout << "m_numVictim " << (App->number_victim) << std::endl;
	...
	/*************************************** verifier la compatibilite entre la position et
	 * l'energie
	 * recu***************************************************************************************************************************/
	...
	/************************************************ savegarder l'energie et la distance dans
	 * param******************************************************************************************************************/
	...
	if ( (bufferTompo[0] == 0) && (bufferTompo[2] == 0) && (bufferTompo[3] == 1) )
	{
		...
		if (params.m_srcAddr.ConvertToInt() < (App->number_victim))
			data[1] = (uint8_t)1;
		else
			data[1] = (uint8_t)0;
		...
		Ptr<LrWpanNetDevice> device = App->GetDevice(parameters.m_dstAddr.ConvertToInt()+App->number_uav);
		uint32_t distance = App->Distance(device->GetPhy()->GetMobility()->GetPosition(), ma_position);

		std::cout << "false sending distance" << distance << std::endl;
		...
		App->SendPositionResponse(parameters, MoiDevReceiver, packet);
	}
	...
	std::cout << "Finish : DataIndication (Malicious) " << std::endl;
...
void
PlmeEdConfirmGPS(Malicious* App, Ptr<Node> drone, LrWpanPhyEnumeration status, uint8_t level)
{
}

void
PlmeEdConfirmGPS_V2(Malicious* App,
					Ptr<Node> drone,
					LrWpanPhyEnumeration status,
					uint8_t level,
					double averagePower,
					...
					double myPRN)
{
	std::cout << "Start : PlmeEdConfirmGPS_V2 : (Malicious)" << std::endl;
	std::cout << "time :" << Simulator::Now().GetSeconds()  << std::endl;
	...
	// mousaab std::cout<<"	 je suis l'uav : ["<< Address <<"] ma position IMU est : x :" <<
	// position.x<<" , y= "<<position.y<< ", z = "<< position.z<< "\n\n";
...
{
	// std::cout<<"_____________________DataConfirmGPS (Malicious)
...
void
DataIndicationGPS(Malicious* App,
				  Ptr<Node> drone,
				  Ptr<RandomWalk2dMobilityModel> RandomMob,
				  McpsDataIndicationParams params,
				  Ptr<Packet> p)
{
	...
	std::cout << "Start : DataIndicationGPS : (Malicious)" << std::endl;
	std::cout << "time :" << Simulator::Now().GetSeconds()  << std::endl;
	// mousaab std::cout<<"_____________________DataIndicationGPS (Malicious)
	// ____________________________________\n ";
	...
	Ptr<MobilityModel> mobility = MoiDevReceiver->GetPhy()->GetMobility();
	position = mobility->GetPosition();
	// mousaab std::cout<<"			je suis le noeud ["<< Address<<"] ma position ( IMU ) est : x :"
	// << position.x<<" , y= "<<position.y<< ", z= "<<position.z<<"\n\n";
	...
	//				App->m_LastRatio= static_cast<double> (RxPower-2); // -2db : a cause de
	//l'attenuation atmospherique

	// std::cout<<"\n 			Satellite position     x =  " << SenderPosition.x<<" , y =  " <<
	// SenderPosition.y<< ", z =  " << SenderPosition.z<<" m \n"; std::cout<<" \n 			distance
	// = " << distance<<"... LrWpan energy = "<<App->m_LastEnergy<<   "... C/N0 = " <<
	// App->m_FixeCN0<<"... Received power= "<<App->m_AbsolutGpsPower<<"\n";
...
Malicious::StartApplication()
{
	...
	// &Malicious::SendGpsSpoofingSignal, this);
					...
					DronParams1 PacketsInfo)
{
	...
	{
	case 0: // spoofing
			// desactiver le recepteur gps afin d'envoyer le le msg spoofing (basculer vers le mode
			// emeteur)
		// MoiDevReceiver->GetPhy()->DisableGPS();

		std::cout << "spoofing =" << g_received << " \n";


		Simulator::Schedule(Seconds(0.0),
							&Malicious::SendGpsSpoofingSignal,
							this,
							params,
							MoiDevReceiver,
							PacketsInfo);
		...
		Simulator::Schedule(Seconds(0.002),
							...
							MoiDevReceiver,
							PacketsInfo);
		Simulator::Schedule(Seconds(0.003),
							&Malicious::SendGpsSpoofingSignal,
							this,
							params,
							MoiDevReceiver,
							PacketsInfo);
		Simulator::Schedule(Seconds(0.004),
							&Malicious::SendGpsSpoofingSignal,
							this,
							params,
							MoiDevReceiver,
							PacketsInfo);
		Simulator::Schedule(Seconds(0.005),
							&Malicious::SendGpsSpoofingSignal,
							this,
							params,
							MoiDevReceiver,
							PacketsInfo);
		Simulator::Schedule(Seconds(0.006),
							&Malicious::SendGpsSpoofingSignal,
							this,
							params,
							MoiDevReceiver,
...
Malicious::SendAck(McpsDataIndicationParams params, Ptr<LrWpanNetDevice> MoiDevReceiver)
{
	...
	Ptr<Packet> Answer = Create<Packet>(data, 4);
...
void
Malicious::JumpToTheMoon(Ptr<LrWpanNetDevice> MoiDevReceiver,
						 Ptr<RandomWalk2dMobilityModel> RandomMob,
						 Ptr<Node> drone)
{
...
void Malicious::SendPositionResponse(McpsDataRequestParams params, Ptr<LrWpanNetDevice> myNetDevice, Ptr<Packet> p)
{
	DronePacketNumber(0);
...
Malicious::SendPackets()
{
	...
	std::cout << "	je suis le noeud malveillant [" << Address
			  << "] ma vrai position  est : x :" << position.x << " , y= " << position.y << "\n\n";
	...
	std::cout << std::endl;
	std::cout << "Msg ennvoye a tt le monde <usurpation d'identite> :" << std::endl;
...
void Malicious::prepareNextAttack(McpsDataIndicationParams params, Ptr<LrWpanNetDevice> MoiDevReceiver, DronParams1 PacketsInfo)
{
	Simulator::Schedule(Seconds(0.1), &Malicious::SendGpsSpoofingSignal, this, params, MoiDevReceiver, PacketsInfo);
}


void
Malicious::SendGpsSpoofingSignal(McpsDataIndicationParams params,
								 Ptr<LrWpanNetDevice> monGPSDevReceiver,
								 DronParams1 PacketsInfo)
{
	SatelliteNumber(0);
	std::cout << "\n\n***********************************************************************\n";
	std::cout << "**********************debut SendPacketsv(GPS "
				 "Spoofing)*********************************\n";
	std::cout << "***********************************************************************\n\n";
	...
	/*
	//authentic PRN code :
	uint32_t authentic[]= {4294967295, 2863311530,
	3435973836,2576980377,4042322160,2779096485,3284386755,2526451350,4278255360,2857740885,3425946675,2573637990,4027576335,2774181210,3275539260};
...
*/
	...
	Simulator::Schedule(Seconds(0),
						&LrWpanMac::McpsDataRequest,
						monGPSDevReceiver->GetMac(),
						paramsAnswer,
						p);
	prepareNextAttack(params, monGPSDevReceiver, PacketsInfo);
