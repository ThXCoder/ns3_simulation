...
Ptr<SpectrumValue>
LrWpanSpectrumValueHelper::CreateSatelliteNoise (uint32_t channel, double noiseFactor)
{
	    //std::cout<<"\n\n		*********************  Debut LrWpanSpectrumValueHelper::CreateNoisePowerSpectralDensity   *************\n";
	  ...
	  //double Nt = BOLTZMANN * 513.0;
	  double Nt = BOLTZMANN * 300.0;
	  // noise Floor (W) which accounts for thermal noise and non-idealities of the receiver
	    //std::cout<<"---------------m_noiseFactor= "<<m_noiseFactor<<"\n";

	  double noisePowerDensity = noiseFactor * Nt/ 1.0e6;
	    //std::cout<<"---------------noisePowerDensity= "<<noisePowerDensity<<"\n";
		//double noise =  10.0 * log10 (noisePowerDensity);
							//std::cout<<	"	noise = " << noise<<"\n" ;
	  NS_ASSERT_MSG ((channel >= 11 && channel <= 26), "Invalid channel numbers");

	  (*noisePsd)[2405 + 5 * (channel - 11) - 2400 - 2] = noisePowerDensity* 0.005;
	  (*noisePsd)[2405 + 5 * (channel - 11) - 2400 - 1] = noisePowerDensity* 0.495;
	  (*noisePsd)[2405 + 5 * (channel - 11) - 2400] = noisePowerDensity;
	  (*noisePsd)[2405 + 5 * (channel - 11) - 2400 + 1] = noisePowerDensity* 0.495;
	  (*noisePsd)[2405 + 5 * (channel - 11) - 2400 + 2] = noisePowerDensity* 0.005;
	    //std::cout<<"\n\n		*********************  fin LrWpanSpectrumValueHelper::CreateNoisePowerSpectralDensity   *************\n";
