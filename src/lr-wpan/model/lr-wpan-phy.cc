...
//    ReceivedPRN = (bufferTompo[34] << 24) + (bufferTompo[33] << 16) + (bufferTompo[32] << 8) + bufferTompo[31];
    ReceivedPRN = (bufferTompo[32] << 24) + (bufferTompo[31] << 16) + (bufferTompo[30] << 8) + bufferTompo[29];


/*
    uint32_t prn = (bufferTompo[32] << 24) + (bufferTompo[31] << 16) + (bufferTompo[30] << 8) +
                  bufferTompo[29];
    uint32_t temp = (bufferTompo[34] << 24); // + (bufferTompo[31] << 16) + (bufferTompo[30] << 8) + bufferTompo[29];
    uint32_t tempor2 = (bufferTompo[33] << 24); // + (bufferTompo[31] << 16) + (bufferTompo[33] << 8) + bufferTompo[34];
    uint32_t tempor3 = (bufferTompo[32] << 24); // + (bufferTompo[31] << 16) + (bufferTompo[33] << 8) + bufferTompo[34];
    uint32_t tempor4 = (bufferTompo[31] << 24); // + (bufferTompo[31] << 16) + (bufferTompo[33] << 8) + bufferTompo[34];

    for (int i = 0; i < 35; i++)
        std::cout << "temp[: " << i << "] : " << (uint32_t)bufferTompo[i] << std::endl;


    std::cout << "recvSize: " << recvSize << std::endl;
    std::cout << "temp: " << temp << std::endl;
    std::cout << "tempor2: " << tempor2 << std::endl;
    std::cout << "tempor3: " << tempor3 << std::endl;
    std::cout << "tempor4: " << tempor4 << std::endl;
    std::cout << "ReceivedPRN: " << ReceivedPRN << std::endl;
    std::cout << "prn: " << prn << std::endl;
    std::cout << "m_PrnChannel: " << m_PrnChannel << std::endl;
*/
    ...
    // Tahar received power : NewAbsolutPower
    // std::cout << "received power : NewAbsolutPower : " << NewAbsolutPower << std::endl;
    ...
    // if (true)
    {
        // std::cout << "ReceivedPRN == m_PrnChannel : ENTERED" << std::endl;
        ...
        // std::cout << "m_trxState == IEEE_802_15_4_PHY_RX_ON && !m_setTRXState.IsRunning() : if" << std::endl;
        // std::cout << "m_trxState == IEEE_802_15_4_PHY_RX_ON = : " << (m_trxState == IEEE_802_15_4_PHY_RX_ON) << std::endl;
        // std::cout << "m_trxState == IEEE_802_15_4_PHY_RX_ON = : " << (m_trxState) << std::endl;
        // std::cout << "!m_setTRXState.IsRunning() = : " << (!m_setTRXState.IsRunning()) << std::endl;
        ...
        // if (true)
        {
            ...
            // std::cout << "m_PrnOfMaxPeak : " << m_PrnOfMaxPeak << std::endl;
...
{
	// std::cout << "Start : StartRx " << std::endl;
    ...
    if ((m_GPSstate == 0) && (m_isSatellite))
    {
	    ...
	    // std::cout << "Start : else (m_GPSstate == 0) && (m_isSatellite) " << ((m_GPSstate == 0) && (m_isSatellite)) << std::endl;

    }
    else 
    {
	    // std::cout << "Start : else (m_GPSstate == 0) && (m_isSatellite) " << ((m_GPSstate == 0) && (m_isSatellite)) << std::endl;
	    // std::cout << "Start : (m_GPSstate == 0)  " << (m_GPSstate == 0) << std::endl;
	    // std::cout << "Start : (m_isSatellite) " << (m_isSatellite) << std::endl;
        // if (m_GPSstate)
        if (m_isSatellite && m_GPSstate)
        {
            StartSatelliteRx(spectrumRxParams);
        }

        // else if ((m_isSatellite ==0) && (m_GPSstate == 1))
        // if ((m_isSatellite == 0))  

        // std::cout << "Start : Node StartRx " << std::endl;
        ...
        if (!m_isSatellite)
        {
            ...
            if (m_trxState == IEEE_802_15_4_PHY_RX_ON && !m_setTRXState.IsRunning())
            {
            	// std::cout << "Start : Node StartRx : Problem : (m_trxState == IEEE_802_15_4_PHY_RX_ON && !m_setTRXState.IsRunning())" << std::endl;
                ...
                //////IDS //mousaab 
                // std::cout<< "" <<this << "receiving packet with power:///" << (10 * log10(LrWpanSpectrumValueHelper::TotalAvgPower (lrWpanRxParams->psd, m_phyPIBAttributes.phyCurrentChannel)) + 30) << "dBm\n";
            ...
            {
            	// std::cout << "Start : Node StartRx : Problem : packet collision" << std::endl;
            ...
            {
            	// std::cout << "Start : Node StartRx : Problem : transceiver not in RX state" << std::endl;
                //////IDS //mousaab std::cout<<"44444 collision + interference (car le recepteur
                /// entraine d'envoyer un msg) \n";
	...
	// std::cout << "Finish : StartRx " << std::endl;
}
...
LrWpanPhy::EndRx(Ptr<SpectrumSignalParameters> par)
{
    ...
    if (params == nullptr)
    {
        ////std::cout<< "Node: " << m_device->GetAddress() << " Removing interferent: " <<
        ///*(par->psd)<<" \n";
...
{
	// std::cout << "Start : PlmeSatelliteEdRequest" << std::endl;
    // IDS //mousaab std::cout<<"**********************debut
	...
	// std::cout << "Finish : PlmeSatelliteEdRequest" << std::endl;
}

void
LrWpanPhy::PlmeEdRequest(void)
{
	std::cout << "Start : PlmeEdRequest" << std::endl;
    ...
    if (m_trxState == IEEE_802_15_4_PHY_RX_ON || m_trxState == IEEE_802_15_4_PHY_BUSY_RX)
    {
        ...
        // std::cout << "results" << m_trxState << std::endl;
    ...
    // std::cout << "m_plmeEdConfirmCallback.IsNull()" << m_plmeEdConfirmCallback.IsNull() << std::endl;
    // std::cout << "!m_plmeEdConfirmCallback.IsNull()" << !m_plmeEdConfirmCallback.IsNull() << std::endl;
    
    ////IDS //mousaab std::cout<<"**********************fin
    /// LrWpanPhy::PlmeEdRequest*********************************\n";
	std::cout << "Finish : PlmeEdRequest" << std::endl;
}
...
{
    // std::cout << "Start : CancelEd" << std::endl; 
    // LrWpanPhy::CancelEd*********************************\n";
...
LrWpanPhy::EndEd(void)
{
    ...
    // std::cout<< "m_plmeEdConfirmCallback_V2.IsNull() = " << m_plmeEdConfirmCallback_V2.IsNull() << "\n";
    // std::cout<< "!m_plmeEdConfirmCallback_V2.IsNull() = " << !m_plmeEdConfirmCallback_V2.IsNull() << "\n";
    ...
    {
        ////IDS //mousaab 
        std::cout<< "energyLevel = " << energyLevel << "\n";
    ...
    {
        //////std::cout<<"			m_plmeEdConfirmCallback est null    >> \n";
    }
    // IDS //mousaab std::cout<<"**********************fin
    // LrWpanPhy::EndEd*********************************\n";
}
