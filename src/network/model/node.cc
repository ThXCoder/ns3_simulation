...
uint32_t
Node::AddLrWpanNetDevice(Ptr<LrWpanNetDevice> device)
{
    // //std::cout<<"========= debut Node::AddLrWpanNetDevice() ======================\n";

    NS_LOG_FUNCTION(this << device);
    uint32_t index = m_LrWpanNetDevice.size();
    m_LrWpanNetDevice.push_back(device);
    device->SetNode(this);
    device->SetIfIndex(index);
    device->SetReceiveCallback(MakeCallback(&Node::NonPromiscReceiveFromDevice, this));
    Simulator::ScheduleWithContext(GetId(), Seconds(0.0), &LrWpanNetDevice::Initialize, device);
    NotifyDeviceAdded(device);
    ////std::cout<<"========= fin Node::AddLrWpanNetDevice() ======================\n";
...
Ptr<LrWpanNetDevice>
Node::GetLrWpanNetDevice(uint32_t index) const
{
    //  //std::cout<<"========= debut Node::LrWpanNetDevice() ======================\n";

    NS_LOG_FUNCTION(this << index);
    NS_ASSERT_MSG(index < m_LrWpanNetDevice.size(),
                  "m_LrWpanNetDevice index " << index << " is out of range (only have "
                                             << m_LrWpanNetDevice.size() << " devices).");

    //  //std::cout<<"		m_LrWpanNetDevice["<<index<<"] = "<<m_LrWpanNetDevice[index]<<"\n";
    ////std::cout<<"========= fin Node::LrWpanNetDevice() ======================\n";

    return m_LrWpanNetDevice[index];
}

std::vector<Ptr<LrWpanNetDevice>>
Node::GetALLLrWpanNetDevice() const
{
    //  //std::cout<<"========= debut Node::LrWpanNetDevice() ======================\n";
    ...
    return m_LrWpanNetDeviceNeighbors;
}

void
Node::AddWpanNetDeviceNeighbors(Ptr<LrWpanNetDevice> LrWpanNetDeviceneighbors)
{
    // std::cout<<"========= debut Node::AddWpanNetDeviceNeighbors() ======================\n";

    NS_LOG_FUNCTION(this << LrWpanNetDeviceneighbors);
    m_LrWpanNetDeviceNeighbors.push_back(LrWpanNetDeviceneighbors);
}
